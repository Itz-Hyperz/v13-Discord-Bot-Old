...
exports.run = async (client, message, args, con) => {
    if(!client.config.economy_module.depositCommand) return message.channel.send({ content: "This module is disabled." }).then((msg) => {
    ...
    if(!args[0]) return message.channel.send({ content: "Please include an amount to deposit." }).then((msg) => {
        if(client.config.deleteCommands) {
            setTimeout(() => {
                msg.delete().catch(e => {});
            }, 14000);
    ...
    if(!amount) return message.channel.send({ content: "Please include an amount to deposit." }).then((msg) => {
        if(client.config.deleteCommands) {
            setTimeout(() => {
                msg.delete().catch(e => {});
            }, 14000);
    ...
    await con.query(`SELECT * FROM users WHERE userid='${message.author.id}'`, async (err, row) => {
        ...
        if(row[0]) {
            ...
            await con.query(`UPDATE users SET balance = balance - ${amount}, bank = bank + ${amount} WHERE userid='${message.author.id}'`, async (err, row) => {
                if(err) throw err;
                message.channel.send({ content: `Deposited \`${client.config.econCurrency}${amount}\` to your bank!` }).then((msg) => {
                    if(client.config.deleteCommands) {
                        setTimeout(() => {
                            msg.delete().catch(e => {});
                        }, 14000);
