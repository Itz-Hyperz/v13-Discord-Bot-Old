...
exports.run = async (client, message, args, con) => {
    ...
    if(message.mentions.users.first()) {
        ...
        return message.channel.send({ content: "I was unable to find that user." }).catch(e => {});
                    ...
                    message.channel.permissionOverwrites.edit(pingeduser, { VIEW_CHANNEL: false, SEND_MESSAGES: false });
                    message.channel.send({ content: `${message.author} removed ${pingeduser.user.tag}. They can no-longer view this ticket or the history.` }).catch(e => {});
        } catch(e) {}
}
